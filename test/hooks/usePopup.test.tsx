...
const CustomizablePopupComponent: React.FC<{ message: string }> = jest.fn(({ message }) => <div>{message}</div>);
...
describe('usePopup', () => {
	it('should render only one popup', () => {
		const { result } = renderHook(() => usePopup(SimplePopupComponent, {}, group), {
	...
	it('should update popup', () => {
		...
		const { result } = renderHook(() => usePopup(CustomizablePopupComponent, {}, group), {
			wrapper: TestHookWrapper,
		});
	...
	it('should merge props', () => {
		const { result } = renderHook(() => usePopup(PopupComponentWithProps, { prop1: 42 }, group), {
			wrapper: TestHookWrapper,
		});
		...
		expect((PopupComponentWithProps as jest.Mock).mock.calls[0][0]).toStrictEqual({
			prop1: 42,
			prop2: 'hello',
		});
	});
